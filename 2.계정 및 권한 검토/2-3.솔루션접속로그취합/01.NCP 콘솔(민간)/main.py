...
def ensure_workbook_with_folder_tabs():
    logging.debug("Create workbook from folder names")
    wb = Workbook()
    default_sheet = wb.active
    sheets_created = False

    for folder_name in sorted(os.listdir(EXCEL_TAB_NAME_DIR)):
        folder_path = os.path.join(EXCEL_TAB_NAME_DIR, folder_name)
        if os.path.isdir(folder_path):
            if not sheets_created:
                default_sheet.title = folder_name
                sheets_created = True
            else:
                wb.create_sheet(title=folder_name)
...
def main():
    ...
    ensure_workbook_with_folder_tabs()
